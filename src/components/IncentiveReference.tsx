...
import { IncentivePackage, IncentiveTier, UpressRate } from '../types/incentive';
...
export default function IncentiveReference({ packages, activeTier, upressRates }: IncentiveReferenceProps) {
  ...
  const upressMap = new Map(upressRates.map((rate) => [rate.packageId, rate]));
  const filteredUpressPackages = filtered.filter((pkg) => upressMap.has(pkg.id));
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden print:hidden">
      ...
      {isOpen && (
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 p-4 sm:p-6 xl:grid-cols-[1.35fr_1fr]">
            <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-800">
              <div className="border-b border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/70">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Insentif SA</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
                    <tr>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400">Paket</th>
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {filtered.map((pkg) => (
                      <tr key={pkg.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        <td className="px-4 py-2.5 font-medium text-gray-900 dark:text-white whitespace-nowrap">{pkg.name}</td>
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-800">
              <div className="border-b border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/70">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Upress Triwulan</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Produk</th>
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-400">10 SA</th>
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-400">15 SA</th>
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-400">20 SA</th>
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-400">25 SA</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {filteredUpressPackages.map((pkg) => {
                      const rate = upressMap.get(pkg.id);
                      if (!rate) return null;
                      return (
            ...
            {filtered.length === 0 && (
              <p className="xl:col-span-2 text-center text-gray-400 dark:text-gray-500 py-6 text-sm">
